...
botonSiguiente.addEventListener('click', () => {
    db.collection('usuarios').orderBy('numero', 'asc').limit(4).startAfter(ultimoDoc).onSnapshot((snapshot) => {
        cargarDocumentos(snapshot.docs)
    });
})
botonAnterior.addEventListener('click', () => {
    db.collection('usuarios').orderBy('numero', 'desc').limit(4).startAfter(primerDoc).onSnapshot((snapshot) => {
